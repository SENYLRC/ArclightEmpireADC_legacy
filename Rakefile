...
namespace :esln_arclight do
  ...
  task :reindex_all do
    ...
    # Identify the configured repos
    #repo_config.keys.each do |#{ENV['FINDING_AID_DATA']}|
      # Index a directory with a given repository ID that matches its filename
      system("DIR=/home/deploy/eads/#{ENV['FINDING_AID_DATA']} REPOSITORY_ID=#{ENV['FINDING_AID_DATA']} rake arclight:index_dir")
    #end
  end
  ...
  task reindex_full_rebuild: %i[arclight:destroy_index_docs esln_arclight:reindex_all] do
